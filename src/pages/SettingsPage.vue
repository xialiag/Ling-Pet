  ...
  <v-layout class="settings-page bg-grey-lighten-5">
    ...
    <v-navigation-drawer permanent width="180">
      ...
      <v-list v-model:selected="selectedTab" nav color="primary" density="compact" class="pa-2">
        <v-list-item v-for="tab in availableTabs" :key="tab.id" :value="tab.id" :prepend-icon="iconMap[tab.icon]"
          :title="tab.name" @click="switchTab(tab.id)" rounded="lg"></v-list-item>
      </v-list>
    </v-navigation-drawer>
