...
// 计算输入框样式，使其跟随 Live2D 模型中心位置
const inputStyle = computed(() => {
  // 当前仅提供 Live2D 模型，因此始终以模型中心为基准
  const modelCenterX = ac.live2dModelPositionX;
  // 将输入框定位在窗口最下方，并在水平方向上跟随模型中心
  // 需要考虑输入框自身的宽度(60%)，所以要居中对齐
  const leftPosition = `${modelCenterX * 100 - 30}%`; // 60% 的一半是 30%
  ...
  return {
    ...
    transform: 'translateX(0)', // 重置默认的居中 transform
    width: `${ac.petSize * 0.6}px`, // 始终占宠物宽度的固定比例
...
.chat-input {
  ...
  bottom: 3%;
  opacity: 0;
  padding: calc(6px * var(--input-scale, 1)) calc(14px * var(--input-scale, 1));
  border: 1px solid rgba(255, 255, 255, 0.3);
  border-radius: calc(15px * var(--input-scale, 1));
