import type { ColorTheme } from '../types/emotion';
import { getEmotionDefinitions } from './emotions';

const DEFAULT_THEME: ColorTheme = {
  background: 'rgba(248, 250, 252, 0.98)',
  border: 'rgba(203, 213, 225, 0.6)',
  text: '#334155',
  shadow: 'rgba(100, 116, 139, 0.15)',
};

const COLOR_MAP: Record<number, ColorTheme> = {
  0: DEFAULT_THEME,
  1: {
    background: 'rgba(254, 249, 195, 0.98)',
    border: 'rgba(252, 211, 77, 0.6)',
    text: '#92400e',
    shadow: 'rgba(245, 158, 11, 0.2)',
  },
  2: {
    background: 'rgba(239, 246, 255, 0.98)',
    border: 'rgba(59, 130, 246, 0.6)',
    text: '#1e40af',
    shadow: 'rgba(59, 130, 246, 0.15)',
  },
  3: {
    background: 'rgba(254, 242, 242, 0.98)',
    border: 'rgba(239, 68, 68, 0.6)',
    text: '#dc2626',
    shadow: 'rgba(239, 68, 68, 0.2)',
  },
  4: {
    background: 'rgba(238, 242, 255, 0.98)',
    border: 'rgba(99, 102, 241, 0.6)',
    text: '#3730a3',
    shadow: 'rgba(99, 102, 241, 0.18)',
  },
  5: {
    background: 'rgba(255, 251, 235, 0.98)',
    border: 'rgba(245, 158, 11, 0.6)',
    text: '#d97706',
    shadow: 'rgba(245, 158, 11, 0.2)',
  },
  6: {
    background: 'rgba(240, 253, 244, 0.98)',
    border: 'rgba(34, 197, 94, 0.6)',
    text: '#15803d',
    shadow: 'rgba(34, 197, 94, 0.15)',
  },
  7: {
    background: 'rgba(253, 244, 255, 0.98)',
    border: 'rgba(196, 181, 253, 0.6)',
    text: '#7c3aed',
    shadow: 'rgba(196, 181, 253, 0.15)',
  },
  8: {
    background: 'rgba(255, 237, 213, 0.98)',
    border: 'rgba(251, 146, 60, 0.6)',
    text: '#c2410c',
    shadow: 'rgba(249, 115, 22, 0.2)',
  },
  9: {
    background: 'rgba(240, 249, 255, 0.98)',
    border: 'rgba(56, 189, 248, 0.6)',
    text: '#075985',
    shadow: 'rgba(56, 189, 248, 0.18)',
  },
  10: {
    background: 'rgba(240, 253, 244, 0.98)',
    border: 'rgba(34, 197, 94, 0.6)',
    text: '#166534',
    shadow: 'rgba(34, 197, 94, 0.15)',
  },
  11: {
    background: 'rgba(255, 247, 237, 0.98)',
    border: 'rgba(251, 146, 60, 0.6)',
    text: '#ea580c',
    shadow: 'rgba(251, 146, 60, 0.2)',
  },
  12: {
    background: 'rgba(254, 243, 199, 0.98)',
    border: 'rgba(245, 158, 11, 0.6)',
    text: '#b45309',
    shadow: 'rgba(245, 158, 11, 0.18)',
  },
  13: {
    background: 'rgba(241, 245, 249, 0.98)',
    border: 'rgba(100, 116, 139, 0.6)',
    text: '#475569',
    shadow: 'rgba(100, 116, 139, 0.15)',
  },
  14: {
    background: 'rgba(236, 254, 255, 0.98)',
    border: 'rgba(6, 182, 212, 0.6)',
    text: '#0e7490',
    shadow: 'rgba(6, 182, 212, 0.18)',
  },
  15: {
    background: 'rgba(252, 231, 243, 0.98)',
    border: 'rgba(244, 114, 182, 0.6)',
    text: '#be185d',
    shadow: 'rgba(236, 72, 153, 0.2)',
  },
  16: {
    background: 'rgba(254, 244, 255, 0.98)',
    border: 'rgba(217, 70, 239, 0.6)',
    text: '#a21caf',
    shadow: 'rgba(217, 70, 239, 0.15)',
  },
  17: {
    background: 'rgba(237, 233, 254, 0.98)',
    border: 'rgba(147, 51, 234, 0.6)',
    text: '#6b21a8',
    shadow: 'rgba(147, 51, 234, 0.15)',
  },
  18: {
    background: 'rgba(248, 250, 252, 0.98)',
    border: 'rgba(148, 163, 184, 0.6)',
    text: '#64748b',
    shadow: 'rgba(148, 163, 184, 0.15)',
  },
};

export function getEmotionColorTheme(emotionCode: number): ColorTheme {
  if (emotionCode in COLOR_MAP) return COLOR_MAP[emotionCode];

  const knownCodes = getEmotionDefinitions().map(item => item.code);
  if (knownCodes.includes(emotionCode)) {
    COLOR_MAP[emotionCode] = DEFAULT_THEME;
    return COLOR_MAP[emotionCode];
  }

  return DEFAULT_THEME;
}
